...
assert.equal(relativePackageManifestStatus.body.repositoryGraph.syncState, "graph-stale");
...
assert.match(stdProjectionStoreText, /^source path:"std\/path\.0"$/m);
...
assert.match(embeddedStdProjectionStatus.body.diagnostics[0].actual, /embedded stdlib source path/);
